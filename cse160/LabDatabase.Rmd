...
First download the latest dwapi package from https://cran.r-project.org/src/contrib/Archive/dwapi/ and the latest data.world package from https://cran.r-project.org/src/contrib/Archive/data.world/ -- I put them in my Downloads folder.

Install the following once; then you can comment out these lines.  Note that you'll need to update the file location if you stored the .tar.gz files somewhere different from where I did.
```{r}
install.packages(c("httr","readr","rjson","xml2")) # these are prerequisites for dwapi package
install.packages("~/Documents/cse160/dwapi_0.1.3.1.tar.gz", repos=NULL, type="source")
install.packages(c("ini","miniUI","shiny")) # these are additional prerequisites for data.world package
install.packages("~/Documents/cse160/data.world_1.2.2.tar.gz", repos=NULL, type="source")
```
...
From the ***R console*** you'll want to run the two commands below after replacing the string with your actual API token string:
```{r configure, eval = FALSE}     
saved_cfg <- data.world::save_config("eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJwcm9kLXVzZXItY2xpZW50OmpqczIyMCIsImlzcyI6ImFnZW50OmpqczIyMDo6MTlkZjhlYzktNTAzZS00ZjMxLWIzYTAtMzU4ZWExMjk4ZGI2IiwiaWF0IjoxNTgyNTcwMzk3LCJyb2xlIjpbInVzZXJfYXBpX3JlYWQiLCJ1c2VyX2FwaV93cml0ZSJdLCJnZW5lcmFsLXB1cnBvc2UiOnRydWUsInNhbWwiOnt9fQ.XoF3TtWtEYKHIvrnEC2jEzrW4U6NaFRwyQUAuoyUeE94lzjgQpDIM7iGXHfk6Tfd_IKkBq3JFyHABPWKlu2RIA")    
...
You might have noticed that the two dataframes above have many player names in common.  Often, we will want to combine two dataframes when they have common rows (so that we end up with a single dataframe that contains attributes from both of the original dataframes).
...
Note that the above code is commented out as it won't work unless you have successfully retrieved the table stats up in line 150.
...
sales <- data.world::query(sale_query, dataset = companydataset)
dim(sales)
head(sales)
```
...
```{r}
salesAgg <- aggregate(. ~ region, sales, sum)
salesAgg$channel <- NULL
salesAgg$totalSales <- rowSums(salesAgg[2:7])
salesAgg
```
...
```{r}
library(ggplot2)
qplot(region, totalSales, data=salesAgg, geom="col")
```
