...
class ASTBuilder extends AsaBaseVisitor[ASTNode] {
  ...
  override def visitSetLiteral(ctx:AsaParser.SetLiteralContext) = {
    val elements = ctx.literal().asScala.toList.map(visit(_))
    val elementTypes = elements.map( (e) => {
      e match {
        case Literal(_, typ, _) => typ
      }
    })
    val baseType = if (elementTypes.distinct.length == 1) elementTypes.head else NO_TYPE
    Literal(SetLiteral(elements), SET_TYPE(baseType), Location(0,0))
  }
    ...
    val start = Integer.decode(ctx.DECIMALINTEGERLITERAL(0).getText)
    val end   = Integer.decode(ctx.DECIMALINTEGERLITERAL(1).getText)
  ...
  override def visitRhsvalue(ctx:AsaParser.RhsvalueContext) = {
    ...
    for (i <- 1 until ctx.relationalexpression.size()) {
      val re2 = visit(ctx.relationalexpression(i))
      val re = Binop(vocabulary.getSymbolicName(ctx.op.get(i-1).getType), re1, re2)
      re1 = re
    }
    re1
  }
    ...
    if (ctx.simpleexpression.size() > 1) { 
      ...
      val b = Binop(vocabulary.getSymbolicName(op), se1, se2)
      b
    } else {
      se1
    }
